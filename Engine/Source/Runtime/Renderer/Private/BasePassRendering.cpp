...
#define IMPLEMENT_BASEPASS_WORKGRAPHSHADER_TYPE(LightMapPolicyType,LightMapPolicyName,bEnableSkyLight,SkyLightName,bVoxel,VoxelName) \
	typedef TBasePassCS<LightMapPolicyType, bEnableSkyLight, bVoxel, SF_WorkGraphComputeNode> TBasePassWorkGraphCS##LightMapPolicyName##SkyLightName##VoxelName; \
	IMPLEMENT_MATERIAL_SHADER_TYPE(template<>,TBasePassWorkGraphCS##LightMapPolicyName##SkyLightName##VoxelName,TEXT("/Engine/Private/BasePassPixelShader.usf"),TEXT("MainCS"),SF_WorkGraphComputeNode);
...
#define IMPLEMENT_BASEPASS_LIGHTMAPPED_SHADER_TYPE(LightMapPolicyType,LightMapPolicyName) \
	...
	IMPLEMENT_BASEPASS_COMPUTESHADER_TYPE(LightMapPolicyType,LightMapPolicyName,true,Skylight,true,Voxel) \
	IMPLEMENT_BASEPASS_COMPUTESHADER_TYPE(LightMapPolicyType,LightMapPolicyName,false,,true,Voxel) \
	IMPLEMENT_BASEPASS_COMPUTESHADER_TYPE(LightMapPolicyType,LightMapPolicyName,true,Skylight,false,) \
	IMPLEMENT_BASEPASS_COMPUTESHADER_TYPE(LightMapPolicyType,LightMapPolicyName,false,,false,) \
	IMPLEMENT_BASEPASS_WORKGRAPHSHADER_TYPE(LightMapPolicyType,LightMapPolicyName,true,Skylight,true,Voxel) \
	IMPLEMENT_BASEPASS_WORKGRAPHSHADER_TYPE(LightMapPolicyType,LightMapPolicyName,false,,true,Voxel) \
	IMPLEMENT_BASEPASS_WORKGRAPHSHADER_TYPE(LightMapPolicyType,LightMapPolicyName,true,Skylight,false,) \
...
{	
	...
	{
		SetDepthStencilStateForBasePass_Internal<bDepthTest, CompareFunction, GET_STENCIL_BIT_MASK(RECEIVE_DECAL, 1) | GET_STENCIL_BIT_MASK(RAY_TRACING_REPRESENTATION, 1) | STENCIL_LIGHTING_CHANNELS_MASK(0x7)>(InDrawRenderState);
	}
}
...
void AddUniformBasePassComputeShader(bool bEnableSkyLight, bool bVoxel, bool bIsDebug, EShaderFrequency ShaderFrequency, FMaterialShaderTypes& OutShaderTypes)
{
	...
	if (ShaderFrequency == SF_Compute)
	{
		if (bVoxel)
		{
			if (bEnableSkyLight)
			{
				OutShaderTypes.AddShaderType<TBasePassCS<TUniformLightMapPolicy<Policy>, true, true, SF_Compute>>(PermutationId);
			}
			else
			{
	...
	else if (ShaderFrequency == SF_WorkGraphComputeNode)
	{
		if (bVoxel)
		{
			if (bEnableSkyLight)
			{
				OutShaderTypes.AddShaderType<TBasePassCS<TUniformLightMapPolicy<Policy>, true, true, SF_WorkGraphComputeNode>>(PermutationId);
			}
			else
			{
...
)
{
	...
	if (ComputeShader)
	{
		AddUniformBasePassComputeShader<Policy>(bEnableSkyLight, bVoxel, bIsDebug, ShaderFrequency, ShaderTypes);
	}
	...
	)
{
	...
	{
	case LMP_PRECOMPUTED_IRRADIANCE_VOLUME_INDIRECT_LIGHTING:
		return GetUniformBasePassShader<LMP_PRECOMPUTED_IRRADIANCE_VOLUME_INDIRECT_LIGHTING>(Material, VertexFactoryType, FeatureLevel, bEnableSkyLight, bVoxel, bIsDebug, ShaderFrequency, ComputeShader);
	case LMP_CACHED_VOLUME_INDIRECT_LIGHTING:
		return GetUniformBasePassShader<LMP_CACHED_VOLUME_INDIRECT_LIGHTING>(Material, VertexFactoryType, FeatureLevel, bEnableSkyLight, bVoxel, bIsDebug, ShaderFrequency, ComputeShader);
	case LMP_CACHED_POINT_INDIRECT_LIGHTING:
		return GetUniformBasePassShader<LMP_CACHED_POINT_INDIRECT_LIGHTING>(Material, VertexFactoryType, FeatureLevel, bEnableSkyLight, bVoxel, bIsDebug, ShaderFrequency, ComputeShader);
	case LMP_LQ_LIGHTMAP:
		return GetUniformBasePassShader<LMP_LQ_LIGHTMAP>(Material, VertexFactoryType, FeatureLevel, bEnableSkyLight, bVoxel, bIsDebug, ShaderFrequency, ComputeShader);
	case LMP_HQ_LIGHTMAP:
		return GetUniformBasePassShader<LMP_HQ_LIGHTMAP>(Material, VertexFactoryType, FeatureLevel, bEnableSkyLight, bVoxel, bIsDebug, ShaderFrequency, ComputeShader);
	case LMP_DISTANCE_FIELD_SHADOWS_AND_HQ_LIGHTMAP:
		return GetUniformBasePassShader<LMP_DISTANCE_FIELD_SHADOWS_AND_HQ_LIGHTMAP>(Material, VertexFactoryType, FeatureLevel, bEnableSkyLight, bVoxel, bIsDebug, ShaderFrequency, ComputeShader);
	case LMP_NO_LIGHTMAP:
		return GetUniformBasePassShader<LMP_NO_LIGHTMAP>(Material, VertexFactoryType, FeatureLevel, bEnableSkyLight, bVoxel, bIsDebug, ShaderFrequency, ComputeShader);
	default:
		check(false);
		return false;
	}
		...
		if (bIsTranslucent && !Material.IsDeferredDecal())
		{
			...
			case ETranslucencyPass::TPT_TranslucencyStandard:
				...
				if (AutoBeforeDOFTranslucencyBoundary > 0.0f)
				{
					...
					bShouldDraw = bShouldDraw || (Material.IsTranslucencyAfterDOFEnabled() && (Material.IsDualBlendingEnabled() || IsModulateBlendMode(BlendMode)));
				}
				break;

			case ETranslucencyPass::TPT_TranslucencyStandardModulate:
				bShouldDraw = !Material.IsTranslucencyAfterDOFEnabled() && !Material.IsTranslucencyAfterMotionBlurEnabled() 
					&& (Material.IsDualBlendingEnabled() || IsModulateBlendMode(BlendMode));
				break;
			...
			case ETranslucencyPass::TPT_TranslucencyAfterDOFModulate:
				bShouldDraw = Material.IsTranslucencyAfterDOFEnabled() && (Material.IsDualBlendingEnabled() || IsModulateBlendMode(BlendMode));
				break;
